...
router.post('/checkin', auth, async (req, res) => {
  try {
    ...
    // Determine status (simple logic: check-in after 9:30 AM is late)
    let status = 'present';
    const checkLimit = new Date(now);
    checkLimit.setHours(9, 30, 0, 0);
    if (now > checkLimit) status = 'late';
...
router.post('/checkout', auth, async (req, res) => {
  try {
    ...
    if (!record) return res.status(400).json({ message: 'No check-in record found for today' });
    ...
    if (record.checkIn) {
      const [h1, m1, s1] = record.checkIn.split(':').map(Number);
      const [h2, m2, s2] = time.split(':').map(Number);
      const start = h1 + m1/60 + s1/3600;
      const end = h2 + m2/60 + s2/3600;
      record.workingHours = parseFloat((end - start).toFixed(2));
      
      // Overtime if > 8 hours
      if (record.workingHours > 8) {
        record.overtime = parseFloat((record.workingHours - 8).toFixed(2));
      }
    }
...
router.get('/today', auth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'employee') {
      const employee = await Employee.findOne({ email: req.user.email });
      if (employee) query.employeeId = employee._id;
    }
