...
const AttendanceDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setTodayAttendance(Array.isArray(todayData) ? todayData[0] : todayData)

      const historyData = historyRes.data.data || historyRes.data || []
      setAttendanceHistory(Array.isArray(historyData) ? historyData : [])

      calculateStats(Array.isArray(historyData) ? historyData : [])
    } catch (error) {
  ...
  return (
    <DashboardLayout title="Attendance Dashboard">
      {/* Header */}
      <Box sx={{
        background: colors?.header?.gradient || 'linear-gradient(135deg, #1976d2 0%, #1565c0 100%)',
        color: 'white',
        p: 3,
        borderRadius: '16px',
        mb: 3,
        ...
        <Button 
          variant="contained" 
          startIcon={<Schedule />}
          onClick={() => navigate('/employee/checkinout')}
          sx={{ 
            bgcolor: 'white', 
