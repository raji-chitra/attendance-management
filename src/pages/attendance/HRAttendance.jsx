...
const HRAttendance = () => {
  ...
  const getUniqueEmployees = () => {
    return employees.map(emp => ({ name: emp.name, id: emp.employeeId }))
  }
  ...
  const getTodayStats = () => {
    ...
    const present = todayRecords.filter(record => ['present', 'late', 'halfday', 'half-day'].includes(record.status)).length
  ...
  const getEmployeeStats = (employeeName) => {
    ...
    const present = employeeRecords.filter(record => ['present', 'late', 'halfday'].includes(record.status)).length
