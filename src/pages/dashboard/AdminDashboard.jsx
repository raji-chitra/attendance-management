    ...
    const loadDashboardData = async () => {
      try {
        ...
        try {
          ...
          const presentCount = records.filter(a => ['present', 'late', 'halfday', 'half-day'].includes(a.status?.toLowerCase())).length || 0
          setStats(prev => ({
            ...prev,
        ...
        try {
          ...
          const activeCount = servicesList.filter(s => s.status === 'in_progress' || s.status === 'pending').length
          setStats(prev => ({
            ...prev,
                ...
                <Box display="flex" alignItems="center" justifyContent="space-between">
                  <Box>
                    ...
                    <Box display="flex" alignItems="center" mt={1}>
                      <TrendingUp
                        sx={{
                          color: card.changeType === 'increase' ? '#2e7d32' : colors.primary.main,
                          mr: 0.5,
                          fontSize: 16
                        }}
                      />
              ...
              <List>
                {recentActivities.map((activity, index) => (
                  <ListItem key={index} alignItems="flex-start">
                    <ListItemAvatar>
                      <Avatar sx={{ bgcolor: colors.primary.main, color: 'white' }}>
                        {activity.icon}
                      </Avatar>
                    </ListItemAvatar>
                    <ListItemText
