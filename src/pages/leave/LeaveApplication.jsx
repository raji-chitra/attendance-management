...
const LeaveApplication = () => {
  ...
  const handleBulkApprove = (requestIds) => {
    setLeaveRequests(prev =>
      prev.map(req =>
        requestIds.includes(req.id || req._id)
  ...
  return (
    <DashboardLayout title={isEmployee ? "Leave Application" : "Leave Management"}>
      {/* Enhanced Header */}
      <Box sx={{
        ...
        <Tabs value={activeTab} onChange={(e, newValue) => setActiveTab(newValue)} variant="scrollable">
          ...
          {isEmployee && (
            <Tab 
              label="My Requests" 
              icon={
                <Badge badgeContent={leaveRequests.length} color="primary" sx={{ '& .MuiBadge-badge': { fontSize: 10, height: 16, minWidth: 16 } }}>
                  <Description />
                </Badge>
              } 
            />
                        ...
                        <TableRow key={request.id || request._id} hover>
                          <TableCell>
                            <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                              #{(request.id || request._id || '').toString().slice(-4).padStart(4, '0')}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Chip
                          ...
                          <TableCell>
                            ...
                            <Typography variant="caption" color="text.secondary" display="block">
                              ...
                              {request.leaveType?.toLowerCase() !== 'permission' && ` to ${new Date(request.endDate).toLocaleDateString()}`}
                            </Typography>
                          </TableCell>
                          <TableCell>
