...
const ServiceList = () => {
  ...
  const getEmployeeName = (assignedTo) => {
    ...
    const employee = employees.find(u => u.id === assignedTo || u._id === assignedTo)
    return employee ? employee.name : 'Unassigned'
  }

  const getEmployeeAvatar = (assignedTo) => {
    if (typeof assignedTo === 'object' && assignedTo !== null) {
      return assignedTo.avatar || ''
    }
    const employee = employees.find(u => u.id === assignedTo || u._id === assignedTo)
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          <Typography variant="h4" fontWeight={700}>
            {isEmployee ? 'My Tasks & Projects' : 'Service Management Hub'}
          </Typography>
        ...
        {(isAdmin || isHR) && (
          <Button
            ...
            sx={{
              bgcolor: colors?.primary?.main || '#1976d2',
              fontWeight: 600,
            }}
          >
            Create Service
